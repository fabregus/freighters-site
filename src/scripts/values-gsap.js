...
function initValues() {
  ...
  const pin = document.getElementById("values-pin");
  if (!wrap || !pin) return;

  const panels = gsap.utils.toArray("#values-pin .values-panel");
  ...
  ScrollTrigger.getAll().forEach((t) => {
    if (t?.vars?.id === "values") t.kill(true);
  });
...
document.addEventListener("astro:page-load", initValues);

// fallback για initial load αν δεν έχεις transitions
if (document.readyState !== "loading") initValues();
else document.addEventListener("DOMContentLoaded", initValues, { once: true });